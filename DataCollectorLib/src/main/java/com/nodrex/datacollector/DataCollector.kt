 ...
 * set of data, you must create a **new instance** of the `DataCollector`.
 *
 ...
 * @param collectorCount The number of times to collect a complete set of data. `null` (the default) means it will collect continuously until `cancel()` is called.
 ...
 */
class DataCollector<T : Any> @PublishedApi internal constructor(
    private val classType: KClass<T>,
    private val onResult: (result: T?, error: Throwable?) -> Unit,
    private val collectorCount: Int? = null,
    ...
    companion object {
         ...
         * generic parameter (e.g., `collect<EventsData>(...)`) without needing to pass the
         ...
         * @return A new, active instance of [DataCollector].
        ...
        ) = DataCollector(T::class, onResult, collectionCount, dispatcher)
         ...
         * @param dispatcher The `CoroutineDispatcher` for the internal scope. Defaults to `Dispatchers.Default`.
         * @return A new, active instance of [DataCollector] that will automatically cancel after one collection.
         * @throws IllegalArgumentException if the provided generic type `T` is not a `data class`.
         */
        @Throws(IllegalArgumentException::class)
        inline fun <reified T : Any> collectSingle(
            noinline onResult: (result: T?, error: Throwable?) -> Unit,
            dispatcher: CoroutineDispatcher = Dispatchers.Default,
        ) = DataCollector(T::class, onResult, 1, dispatcher)
    }
    ...
    private fun startCollector() {
        ...
        collectorJob = scope?.launch {
            ...
            Util.log("All flows[${currentFlowHolders.size}] are zipped! Started listening for emits...")
            ...
            zippedFlow.collect { resultPairs ->
                ...
                if (collectorCount != null && collectedTimes >= collectorCount) {
                    Util.logw("Maximum number[$collectorCount] was collected, canceling collector.")
     ...
     * for the first available emit for each property.
     *
     ...
     * of another. A future release will introduce a `GroupedDataCollector` to handle these
    ...
    private fun logInitialization() {
        ...
        |$properties
        || Collection Count: [${collectorCount ?: "unlimited"}]
     ...
     * `DataCollector` instance should be considered inactive and should not be used again.
     */
    fun cancel() {
        emitterJob?.cancel()
        emitterJob = null
